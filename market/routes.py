    ...
    if request.method == "POST":
        ...
        if purchased_item_obj:
            if current_user.can_purchase(purchased_item_obj):
                ...
                flash(f"Congratulation! You purchased {purchased_item_obj.name} for {purchased_item_obj.price}",
                      category='success')
            else:
                flash(f"Unfortunately! You don't have enough money to purchase {purchased_item_obj.name}",
                      category='danger')
        # Sell Item Logic
        ...
        if sold_item_obj:
            if current_user.can_sell(sold_item_obj):
                ...
                flash(f"Congratulation! You sold {sold_item_obj.name} back to market!", category='success')
            else:
